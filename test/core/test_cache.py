...
async def test_sqlite_sync_order_accepts_decimal_price(
    tmp_path, task_manager, message_bus
):
    cache = AsyncCache(
        strategy_id="decimal-price-test",
        user_id="auto-test-user",
        msgbus=message_bus,
        task_manager=task_manager,
    ...
    order = Order(
        id="test-order-decimal-price",
        uuid="test-uuid-decimal-price",
        exchange=ExchangeType.BINANCE,
        symbol="BTCUSDT-PERP.BINANCE",
        side=OrderSide.BUY,
        type=OrderType.LIMIT,
