...
class BybitOrderManagementSystem(OrderManagementSystem):
    ...
    def _schedule_cancel_rejected_order_reconcile(
        ...
        self._task_manager.create_task(
            ...
            name=f"bybit-cancel-reconcile-{oid}",
        )
    ...
    def _parse_order_cancel(self, raw: bytes):
        ...
        if msg.is_success:
            ...
            if self._is_cancel_reconcile_error(msg.error_msg):
                self._schedule_cancel_rejected_order_reconcile(
                    oid, tmp_order, msg.error_msg
                )
                return
            order = Order(
                exchange=self._exchange_id,
            ...
            for symbol in candidate_symbols:
                for oid in self._cache.get_symbol_orders(symbol):
                    ...
                    try:
                        ...
                        self._log.warning(
                            f"Bybit reconnect recent-trade check failed for {symbol}#{oid}: {e}"
                        )
