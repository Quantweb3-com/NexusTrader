...
from nexustrader.constants import AccountType, ConfigType, InstrumentType
from nexustrader.schema import InstrumentId, Limit, LimitMinMax, MarginMode, Precision
    ...
    def load_markets(self):
        for raw_market in self._fetch_raw_markets():
            try:
                ...
                self._log.warning(
                    f"Symbol Format Error: {exc}, "
                    f"{raw_market.get('instId')}, {raw_market}"
                )
                continue
        ...
        return OkxMarket(
            ...
            limits=Limit(
                leverage=LimitMinMax(
                    min=1.0, max=self._to_float(raw_market.get("lever"))
                ),
                amount=LimitMinMax(
                ...
                price=LimitMinMax(
                    min=self._to_float(raw_market.get("tickSz")), max=None
                ),
                cost=LimitMinMax(
                    min=None, max=self._to_float(raw_market.get("maxLmtAmt"))
                ),
            ),
