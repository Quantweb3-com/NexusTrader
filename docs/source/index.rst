...
- **Lightweight Core Runtime**:
  Core infrastructure such as the MessageBus, Clock, and logging stack now runs on lightweight pure-Python components plus ``loguru`` (zero build requirements, built-in time-based log rotation), avoiding heavy Rust build requirements while keeping live-trading behavior predictable.
